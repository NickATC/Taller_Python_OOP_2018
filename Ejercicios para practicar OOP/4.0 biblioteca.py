...
class Biblioteca:
    ...
    def mostrar_libros_disponibles(self):
        ...
        for i in #ingrese código para acceder al atributo libros_disponibles#:
            print(i)
        print("")

    def prestar_libro(self, libro_solicitado):
        #libro solicitado esta en lista?
        if libro_solicitado in #ingrese código para acceder al atributo libros_disponibles:   
            print("Ahora usted tiene ese libro por 8 días.")
            self.libros_disponibles.remove(libro_solicitado)  #Retiro el libro de la lista de libros_disponibles
        else:
            print("Lo siento, ese libro ya no está disponible")

    def agregar_libro(self, libro_regresado):
        self.libros_disponibles.append(libro_regresado)   #Agrego libro solicitado a la lista.
...
while True:
    ...
    if opcion is 1:
        #Ingrese código para ver libros disponibles#
    elif opcion is 2:
        libro_solicitado = cliente.pedir_libro()   
        biblioteca.prestar_libro(libro_solicitado)
    elif opcion is 3:
        libro_regresado = #Ingrese código para regresar libro (Cliente)#
        #Ingrese código para agregar libro (Biblioteca)#
    elif opcion is 4:
        quit()
    else:
